...
void testChannel(uint8_t channel) {
  ...
  pwm.setPWM(channel, 0, SERVO_MIN_PULSE);
  delay(1000);
  ...
  moveServoSmooth(channel, SERVO_MAX_PULSE, SERVO_MIN_PULSE);
  delay(1000);
...
void setup() {
  Serial.begin(115200);
  Serial.println("PCA9685 channels 12, 13, 14 sequence test starting...");
...
void loop() {
  for (uint8_t i = 0; i < sizeof(CHANNELS); ++i) {
    testChannel(CHANNELS[i]);
  }

  Serial.println("Sequence complete. Restarting in 3 seconds.");
  delay(3000);
}
